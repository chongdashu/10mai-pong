...
values ('template', 'Template Game')
on conflict (game_key) do update
set display_name = excluded.display_name;

-- Create game-specific view for high scores
create or replace view public.template_high_scores as
select
  u.username,
  gs.score,
  gs.metadata,
...
where g.game_key = 'template'
order by gs.score desc;

-- Grant permissions
grant select on public.template_high_scores to anon, authenticated;
